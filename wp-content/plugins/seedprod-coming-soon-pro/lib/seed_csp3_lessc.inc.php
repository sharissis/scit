 ...
 *
 * LESS css compiler, adapted from http://seed_csp3_lesscss.org
 *
 ...
 * by `seed_csp3_lessc_parser` into a tree, then compiled to CSS text by `seed_csp3_lessc`. The
 ...
 *
 * The `seed_csp3_lessc` class creates an intstance of the parser, feeds it LESS code, then
 * compiles the resulting tree to CSS
 *
 * The `seed_csp3_lessc_parser` class is only concerned with parsing its input.
 *
 * The `seed_csp3_lessc_formatter` classes are used to format the output of the CSS,
 * controlling things like whitespace and line-breaks.
 */
class seed_csp3_lessc {
	...
	function mixImport($import, $parentBlock, &$props) {
		...
		if (empty($media) && substr_compare($url, '.css', -4, 4) !== 0) {
			...
			if (!is_null($realPath)) {
				...
				$parser = new seed_csp3_lessc_parser($this, $realPath);
	 ...
	 *
	 * See seed_csp3_lessc::compileProp()
	 *
	 */
	...
	protected function createChild($fname) {
		$less = new seed_csp3_lessc($fname);
	...
	protected function injectVariables($args) {
		$this->pushEnv();
		$parser = new seed_csp3_lessc_parser($this, __METHOD__);
	...
	function parse($str = null, $initialVariables = null) {
		...
		$this->parser = new seed_csp3_lessc_parser($this, $name);
	...
	function newFormatter() {
		$className = "seed_csp3_lessc_formatter";
		if (!empty($this->formatterName)) {
			if (!is_string($this->formatterName))
				return $this->formatterName;
			$className = "seed_csp3_lessc_formatter_$this->formatterName";
		}
...
class seed_csp3_lessc_parser {
	...
	function __construct($seed_csp3_lessc, $sourceName = null) {
		// reference to less needed for vPrefix, mPrefix, and parentSelector
		$this->seed_csp3_lessc = $seed_csp3_lessc;

		$this->sourceName = $sourceName; // name used for error messages

		if (!self::$operatorString) {
			self::$operatorString =
				'('.implode('|', array_map(array('seed_csp3_lessc', 'preg_quote'),
	 ...
	 *
	 * Consider the function seed_csp3_lessc::keyword(). (all parse functions are
	 * structured the same)
	 *
	 ...
	 *
	 * All of these parse functions are powered by seed_csp3_lessc::match(), which behaves
	 ...
	 * avoid this, seed_csp3_lessc::seek() is used to remember and set buffer positions.
	 *
	...
	protected function parseChunk() {
		...
		if ($this->literal('}')) {
			...
			if (!isset($block->args)) foreach ($block->tags as $tag) {
				if (!is_string($tag) || $tag{0} != $this->seed_csp3_lessc->mPrefix) {
					$hidden = false;
					break;
				}
			}
	...
	protected function fixTags($tags) {
		...
		foreach ($tags as &$tag) {
			if ($tag{0} == $this->seed_csp3_lessc->vPrefix)
				$tag[0] = $this->seed_csp3_lessc->mPrefix;
		}
		return $tags;
	}
	...
	protected function expressionList(&$exps) {
		...
		$exps = seed_csp3_lessc::compressList($values, ' ');
		return true;
	}
	...
	public function propertyValue(&$value, $keyName=null) {
		...
		$value = seed_csp3_lessc::compressList($values, ', ');
		return true;
	}
	...
	protected function tagBracket(&$value) {
		...
		if ($this->literal('[') && $this->to(']', $c, true) && $this->literal(']', false)) {
			...
			$value = str_replace($this->seed_csp3_lessc->parentSelector, "$&$", $value);
			return true;
		}
	...
	protected function variable(&$name) {
		$s = $this->seek();
		if ($this->literal($this->seed_csp3_lessc->vPrefix, false) &&
			($this->variable($sub) || $this->keyword($name)))
		{
			if (!empty($sub)) {
				$name = array('variable', $sub);
			} else {
				$name = $this->seed_csp3_lessc->vPrefix.$name;
			}
			return true;
		}
	...
	protected function literal($what, $eatWhitespace = true) {
		...
		return $this->match(seed_csp3_lessc::preg_quote($what), $m, $eatWhitespace);
	}
	...
	protected function to($what, &$out, $until = false, $allowNewline = false) {
		...
		if (!$this->match('('.$validChars.'*?)'.seed_csp3_lessc::preg_quote($what), $m, !$until)) return false;
...
class seed_csp3_lessc_formatter {
	public $indentChar = "  ";
...
class seed_csp3_lessc_formatter_compressed extends seed_csp3_lessc_formatter {
	public $indentChar = "";
...
class seed_csp3_lessc_formatter_indent extends seed_csp3_lessc_formatter {
	function indentAmount($block) {
