...
$ajax_url = html_entity_decode(wp_nonce_url('admin-ajax.php?action=seed_csp3_import_settings','seed_csp3_import_settings'));
...
echo "<script>
      jQuery(document).ready(function($) {
        $('#import-settings-btn').click(function() {
            if(confirm(seed_csp3_msgs.import_confirm)){
                ...
                $.post('{$ajax_url}',{settings: settings}, function(data) {
                	if(data == '1'){
                    	...
                    	setTimeout('csp3_reload_page()',2000);
                  	}
                });
            }
        }); 
