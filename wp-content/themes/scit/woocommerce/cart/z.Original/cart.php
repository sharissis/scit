						...
						<td class="product-name">
							<?php
								...
								if ( ! $_product->is_visible() || ( $_product instanceof WC_Product_Variation && ! $_product->parent_is_visible() ) )
									{
										//	Link to EVENT if it is an Event... so check against the Category and if it's a ticket do this:
										$product_id = $values['product_id'];
										$event_id = get_post_meta(  $product_id, '_tribe_wooticket_for_event', true );
										$event_title = get_the_title( $event_id );
										if ( !empty( $event_id ) )
										{
											$event_date		= tribe_get_start_date($event_id, false, 'l, F d');
											$event_title	= '<br/><span class="tiny-text">'.$event_title.'<br/>'.$event_date.'</span>';
										}
										echo apply_filters( 'woocommerce_in_cart_product_title', $_product->get_title(), $values, $cart_item_key ).$event_title;
									}
								...
								else
									{
										...
										printf('<a href="%s">%s</a>', esc_url( get_permalink( apply_filters('woocommerce_in_cart_product_id', $values['product_id'] ) ) ), apply_filters('woocommerce_in_cart_product_title', $_product->get_title(), $values, $cart_item_key ) ).$event_title;
									}
